...
export default ActivityItem = ({ image, title, rating, duration, location, before, current, desc, onPress }) => (
    <TouchableOpacity onPress={() => onPress()} style={styles.itemContainer}>
        <ImagePlaceholder duration={1000} source={image} style={styles.image}/>
        <View style={styles.centerItemContainer}>
            <Text numberOfLines={1} style={styles.titleText}>{title}</Text>

            {location != null && <View style={styles.locationContainer} >
            ...
            {location != null && <View style={styles.locationContainer} >
                {/* <Ionicons name="ios-pin" size={12} color={Color.lightText} /> */}
                <Text style={[styles.locationText,{marginLeft:0}]}>Duration : {duration}</Text>
            </View>}
