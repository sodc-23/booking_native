...
export default class FlightItem extends React.PureComponent {
    ...
    renderTicket(itemSrc) {
        if ( itemSrc == null ) return;
        let {item = [{}], properties = {}} = itemSrc
        ...
        let {tpExtension = [], locationInfo = {}, dateInfo = {}, images = [], vendors = [], config, stops = 0, journeyDuration} = item[0]    
                ...
                <View style={styles.titleContainer}>
                    ...
                    <View style={{flex:1}}>
                        <ShimmerPlaceHolder autoRun style={styles.middleText} visible={isFetched}>
                            <Text numberOfLines={1} style={styles.title}>{vendors[0]?vendors[0].item.name:'airlines'}</Text>
                            {/* <Marquee style={styles.title}>{vendors[0]?vendors[0].item.name:'airlines'}</Marquee> */}
                        </ShimmerPlaceHolder>
                    </View>
                </View>
                <View style={styles.lineContainer}>
