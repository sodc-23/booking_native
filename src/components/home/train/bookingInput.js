...
export default class TrainBookingInput extends Component {
    state = {
        items: []
    }
    ...
    A2B() {
        ...
        value.travelers.map((traveler, index) => {
            ...
            items.push({
                type: 'DATE', title: 'Birth Date', value: birthDate, isMandatory: true,errorMsg:"Please select birthdate.",
            ...
            items.push({
                type:'DROPBOX', title:'Pickup Location', value: '', options:[],
                onChnage:(index)=>{

                }, isMandatory:true, pointer:{index, name:'pickup'}
            })
            items.push({
                type:'DROPBOX', title:'Dropoff Location', value: '', options:[],
                onChnage:(index)=>{
            ...
            items.push({
                type: 'PHONE_NUMBER', title: 'Mobile Number', value: { code: '', number: '' }, errorMsg:value=>value.number.isValidPhoneNumber(),
