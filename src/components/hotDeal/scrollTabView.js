                ...
                <ScrollView 
                    ...
                    {titles.map((title, index)=>(
                        <TouchableOpacity ref={e=>this.ref[index] = e} key={index} 
                            style={[styles.tabContainer,length <= 4?{width:screenWidth/length}:{}]} 
                            onPress={()=>this.clickItem(title, index)}>

                            <Text style={[styles.tabText, {color:index==selected?Color.orange:Color.lightPrimary}]}>{T2(title.toLowerCase()).toUpperCase()}</Text>
                        </TouchableOpacity> 
                    ))}
                </ScrollView>
